...
import app.Exeption.ValueExeption;
import app.collectionClasses.Coordinates;
import app.collectionClasses.Location;
...
public class RouteCreator {
    ...
    public Float distanceCreator() {
        ...
        try {
            float i = 1;
            Output.print("Введите расстояние между пунктами:");
            distance = Float.parseFloat(scanner.nextLine());
            if (distance.compareTo(i) <= 0) {
                throw new IllegalArgumentException();

            }
        } catch (IllegalArgumentException e) {
            Output.println("Дистанция не может быть меньше 1" + "  " + e.getMessage());
