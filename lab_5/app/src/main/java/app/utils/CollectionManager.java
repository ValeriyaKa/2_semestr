...
public class CollectionManager {
    ...
    public void removeRoute(long routeId) {
        collection.removeIf(route -> route.getId() == routeId);
    }
    ...
    public void loadCollection() {
        ...
        csvFile.loadFile();
        Set<Route> routes = csvFile.getCollection();
        if (!routes.isEmpty()) {
            collection.addAll(routes);
            Output.println("Данные успешно загружены!");
        } else {
            Output.println("Файл с данными пустой!");
        }
    }
